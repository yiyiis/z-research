...
## Agent 评估（LLM-as-Judge 自动打分）

报告生成完成后，自动用一个 LLM 当评委给报告打分（对齐 RAGAS 思路）。

### 5 个评估维度（每维 0-10 分）

| 维度 | 评什么 |
|---|---|
| **完整性** coverage | 是否覆盖了查询的核心方面 |
| **引用质量** citation | [n] 引用是否充分、是否对应真实来源 |
| **结构组织** structure | 章节是否合理、逻辑是否递进 |
| **客观性** objectivity | 是否避免主观臆断、基于资料 |
| **可读性** readability | 语言是否流畅、术语是否一致 |

### 关键设计

- 评委只看"报告 + 来源列表"，**不引入自己的知识**（避免与被评模型同源偏见）
- 每个维度给出**具体扣分原因**（指出哪一段/哪个论断有问题）
- 失败不致命：评估出错时静默，不影响主流程
- 持久化到 `report_evaluations` 表（一份报告可多次评估，支持重评）

### 触发与展示

- **done 帧之后异步执行**（不阻塞报告展示，评估完成推 `evaluation` 帧）
- 前端 `EvaluationBadge` 组件展示：综合分徽章（≥8 绿/>=6 黄/<6 红）+ 5 维度进度条 + 扣分原因
- CLI 模式打印到 stderr：`📝 评估: 综合 8.2/10`

配置：`EVALUATE_ON_DONE=true`（默认开启，关闭可省一次 LLM 调用）

## 报告对话式修改

报告生成后，通过 `/ws/revise` 端点做多轮对话式修改（像 ChatGPT 那样）。

### 4 类修改能力（用户自然语言指令触发，引擎自动分类）

| 类型 | 例子 | 处理路径 |
|---|---|---|
| **补充检索** supplement | "补充最新的 MoE 技术" | 先调 researcher.Conduct 检索 → 整合进报告 |
| **局部修改** local_edit | "把结论改简洁""扩第三章" | 直接调 LLM 修改 |
| **翻译/改风格** restyle | "翻译成英文""改学术风格" | 直接调 LLM 修改 |
| **多轮迭代** | "刚才太啰嗦了，再精简" | 保留对话历史，理解上下文 |

### 关键设计

- **独立 WS 端点** `/ws/revise`（不动现有 `/ws`，风险隔离）
- 修改后的报告**另存为新记录**（不覆盖原报告），query 标注 `[修订自 #N]`
- 补充检索的新来源编号**顺延**（保留原 [n] 引用有效性）
- 多轮对话历史由前端维护（每次发送带完整 history）

### 协议

客户端 → 服务端首帧：
```json
{"type":"revise_request","report_id":123,"instruction":"把结论改简洁","history":[...]}
```

服务端 → 客户端：
- `revise_progress`（分类/检索进度）
- `revise_sources`（新增来源，仅 supplement）
- `revise_chunk`（流式修改报告，累积值）
- `revise_done`（完成，含新 report_id + action 类型）

前端 `ReviseInput` 组件：输入框 + 对话历史折叠 + 进度日志。
